        ...
        <>
            <s.Item
                ...
                onClick={() => {
                    ...
                    let target
                    /**
                     * We don't have a reliable way to target arbitrary components.
                     * Either the IDs need to stabilize, or we do a text search.
                     * For now, let's look for the text and find its root component, and scroll to that.
                     */
                    const match = document.evaluate(
                        `(//*[not(self::script)][text()='${text?.trim()}'])[2]`,
                        document,
                        null,
                    ...
                    if (match) {
                        const closestRoot =
                            match.closest('[data-scroll-parent]') ??
                            match.closest('[data-root]')
                        if (closestRoot) {
                            target = closestRoot
                        } else {
                            target = match
                    ...
                    if (!target) {
                        console.log("Couldn't find target to scroll to")
                        return
                    }
