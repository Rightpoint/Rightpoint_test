...
export const ScrollInParallax = ({ children, transition }) => {
    ...
    const [distanceToTranslate, _] = useState(Math.floor(Math.random() * 700))

    const transform = useTransform(
        progress,
        [0, 1],
        [`translateY(${distanceToTranslate}px)`, 'translateY(0px)'],
        {}
    )
    const stylesFirst = {
        transform,
    ...
    return (
        <motion.div style={stylesFirst} ref={ref} transition={transition || {}}>
            {children}
        </motion.div>
    )
}
