...
const logLocalDevOnlyBound = createLogLocalDevOnlyFn({
    enabled: true,
    namespace: 'SolutionsAnimation',
    logLevel: 'debug',
})
/**
    ...
    useEffect(() => {
        if (lottieRef.current) {
            ...
            if (!isNavbarPopupOpen && inView) {
                ...
                logLocalDevOnlyBound('Starting lottie animation')
            } else {
                lottieRef.current?.pause?.()
                logLocalDevOnlyBound('Stopping lottie animation')
            }
        }
