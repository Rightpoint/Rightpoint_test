...
export type LandingPageEntryFields = {
    ...
    pardot: any // Fixed naming here
}

export type PardotFormEntryField = {
    description: Document
    fields: PardotFormField[]
    formHandler: EntryFields.Text
}

export type PardotFormField = {
...
>({
    ...
    entryToProps: async ({ entry, manager }) => {
        ...
        const pardotFormEntry: PardotFormEntryField = {
            description: await contentfulRichTextDocumentToJsonSafe(
                pardot?.fields.description,
                manager
            ),
            formHandler: pardot?.fields.formHandler,
            fields: Array.isArray(pardot?.fields.fields)
                ? await Promise.all(
                      pardot.fields.fields.map(async (field) => ({
                          label: field.fields.label,
                          internalName: field.fields.internalName,
