    ...
    implements PageMapperInterface<P>
{
    ...
    public async getPageGlobalsProps() {
        return this.sanitizeJsonResponse({
            ...
            noNavbar: this.nextContext?.params?.noNavbar,
        })
    }
    ...
    public async getCardProps() {
        if (this.config.entryToCardProps) {
            try {
                ...
                console.log('Failed generating card props')
                return {}
            }
        }
        return null
     ...
     * Get SEO props
     *
     * 1: pages can supply SEO defaults via config
     * 2: every page has an SEO entry that overrides everything
     */
