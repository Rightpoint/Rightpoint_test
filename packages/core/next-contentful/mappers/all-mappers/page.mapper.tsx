...
import type { PageMapperInterface } from './mapper.interface'
    ...
    implements PageMapperInterface<P>
{
    ...
    public async getSeoProps() {
        ...
        const seoDefaults = merge(
            {
                /**
                 * Consider adding canonical to all pages, because future
                 * rewrite rules may change the URL and forget to set one.
                 */
                // canonicalUrl: await this.getUrl(),
            },
        ...
        if (seoEntry) {
            try {
                ...
                return this.sanitizeJsonResponse(
                    merge(seoDefaults, await seoMapper.getProps())
                )
            } catch (ex) {
                console.error('Error getting seo props', ex)
                return this.sanitizeJsonResponse(seoDefaults)
            }
        }
        return this.sanitizeJsonResponse(seoDefaults)
    }
