...
import { createMultiMediaVideo } from '../../utils/create-multi-media-and-asset'

export const importVideoSection = async ({
    section,
    scraper,
    environment,
}: {
    section: ReturnType<typeof sectionParsers.video>
    ...
    const fields: Partial<WorkDetailMediaEntry['fields']> = {
        internalName: `Work/${scraper.headerData.title} - Video`,
        a11yLabel: section.title,
        // container: "Container",
        media: [
    ...
    const id = deterministicId(scraper.headerData.title, section.title)
