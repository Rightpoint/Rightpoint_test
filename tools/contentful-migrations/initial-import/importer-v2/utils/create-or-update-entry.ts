...
type EntryArg = {
    ...
    // can provide a raw entry to update
    entry?: Entry
}
/**
 * Create or update an entry
 * given an ID (which can use deterministicId() to generate MD5 hashed IDs)
 * or a raw entry if one has already been fetched.
 */
export async function createOrUpdateEntry({
    ...
    const entryToUpdate = entry
        ? entry
        : await environment.getEntry(id).catch((ex) => {})
    ...
    if (entryToUpdate) {
        ...
        entryToUpdate.fields = transformToLocale(fields)
        entryToUpdate.metadata = metadataTags

        return await entryToUpdate
            .update()
            .then((entry) => {
                const entryContentTypeId = entry.sys.contentType.sys.id
                if (entryContentTypeId !== contentTypeId) {
                    // let's catch egregious errors
                    throw new Error(
...
export async function createOrUpdateAsset({
    ...
    const newAsset = await environment
        .createAssetWithId(id, {
            fields: transformToLocale(fields),
            metadata: metadataTags, // add import tags
        })
        // process images
        .then((asset) => {
