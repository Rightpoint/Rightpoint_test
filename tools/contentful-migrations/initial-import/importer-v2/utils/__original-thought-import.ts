...
const ENVIRONMENT_ID = 'master-01-25-2023-thought-import'
...
const createDocumentWithAssets = async (environment, { url }) => {
    const scrapeData_ = await scrapeThoughtData(url)
    const scrapeData: Exclude<typeof scrapeData_, any[]> = scrapeData_ as any
    const documentWithAssets = await replaceImagesTextWithAssets(
        scrapeData.document,
    ...
    console.log('Done converting to document with assets')
    console.log(' ')
    return [documentWithAssets, scrapeData]
}
...
async function migrateOneThoughtEntry(
    ...
    // before, we were creating a long rich text entry
    ...
    // now, let's just generate a document with assets and move it to the main entry.
    const [documentWithAssets, scrapeData] = await createDocumentWithAssets(
        environment,
        {
            url: url,
        }
     ...
     * Generate author
     */
    const personEntry = await getOrCreatePerson(environment, {
        scrapeData,
    })
    ...
    const fields: Partial<ThoughtDetailEntry['fields']> = {
        ...
        authors: [createLink(personEntry as any, 'Entry')],
    }
